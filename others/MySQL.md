...
| **类型**      | **字节数** | **范围（SIGNED）**               | **范围（UNSIGNED）**               | **说明**                     |
|---------------|------------|-----------------------------------|-----------------------------------|------------------------------|
| TINYINT       | 1          | -128 到 127                     | 0 到 255                         | 小整数                       |
| SMALLINT      | 2          | -32,768 到 32,767               | 0 到 65,535                      | 较小整数                    |
| MEDIUMINT     | 3          | -8,388,608 到 8,388,607         | 0 到 16,777,215                  | 中等大小整数                |
| INT (INTEGER) | 4          | -2,147,483,648 到 2,147,483,647 | 0 到 4,294,967,295               | 标准整数                    |
| BIGINT        | 8          | -2^63 到 2^63-1                 | 0 到 2^64-1                      | 大整数                      |
| FLOAT(M, D)   | 4          | 近似值                          | 近似值                          | 单精度浮点数（可能有误差）   |
| DOUBLE        | 8          | 近似值                          | 近似值                          | 双精度浮点数（可能有误差）   |
| DECIMAL(M, D) | 变长       | 精确值                          | 精确值                          | 定点数，用于精确计算         |
...
| DATETIME   | 8          | YYYY-MM-DD HH:MM:SS    | 1000-01-01 00:00:00 到 9999-12-31 23:59:59 | 日期和时间                  |
| TIMESTAMP  | 4          | YYYY-MM-DD HH:MM:SS    | 1970-01-01 00:00:01 UTC 到 2038-01-19 03:14:07 UTC | 时间戳                     |
...
| **类型**     | **最大长度**        | **说明**                                   |
|--------------|---------------------|--------------------------------------------|
| CHAR(M)      | 0 到 255 字符       | 固定长度字符串，长度不足时用空格填充       |
| VARCHAR(M)   | 0 到 65535 字符     | 可变长度字符串，长度受限于行大小           |
| TINYTEXT     | 255 字节            | 非常小的文本                              |
| TEXT         | 65,535 字节         | 一般文本                                  |
| MEDIUMTEXT   | 16,777,215 字节     | 较大文本                                  |
| LONGTEXT     | 4,294,967,295 字节  | 超大文本                                  |
| TINYBLOB     | 255 字节            | 非常小的二进制数据                        |
| BLOB         | 65,535 字节         | 一般二进制数据                            |
| MEDIUMBLOB   | 16,777,215 字节     | 较大二进制数据                            |
| LONGBLOB     | 4,294,967,295 字节  | 超大二进制数据                            |
| ENUM         | 1 到 2 字节         | 枚举类型，值为预定义的一组值之一          |
| SET          | 1 到 8 字节         | 集合类型，可包含预定义值的任意组合        |
...
**注意事项**
1. **选择数据类型时，尽量根据实际需求选择最适合的类型**，避免浪费存储空间或降低性能。
2. **字符集和排序规则**：字符串类型支持指定字符集（如 `utf8mb4`）和排序规则（如 `utf8mb4_unicode_ci`）。
