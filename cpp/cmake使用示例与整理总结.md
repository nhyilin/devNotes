...
# cmake生成Xcode、vs项目
1. 生成Xcode项目：`cmake -G "Xcode" .`，`.`代表当前目录，即CMakeLists.txt所在的目录，建议最好新建Xcode文件夹，并在Xcode文件夹内执行`cmake -G "Xcode" ../`。如果报错找不到编译器，可以使用：`.sudo xcode-select --switch /Applications/Xcode.app/`，若要配置arm64，则需要如下配置：`cmake -G "Xcode" -DCMAKE_OSX_ARCHITECTURES='arm64' ../CMakeLists.txt`
