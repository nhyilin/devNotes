...
#     data_array = np.array(cols_data)
#     data_list = data_array.astype(float)# numpy强制类型转换
#     return data_list
...
def readFile(path, colsNum, arrayCols):
    with open(path, 'r') as csvfile:
        ...
        index = 0
        column_data = []
        data_list = [None for i in range(arrayCols)]
        i = 1
...
def arrangeMeanAndVariance(path):
    testPoint = readFile(path,COLS_TP,POINTCOLS)
