	...
	,thread(new QThread)
{
	...
	this->connect(thread, SIGNAL(started()), this, SLOT(slotsimPublisher()));
	this->connect(thread, SIGNAL(finished()), this, SLOT(deleteLater()));
...
void MyPublisherFullSpeedThread::slotsimPublisher()
{
	...
	while (!m_stop)
	{
		...
		if (m_pause) { continue; }
		if (!(FeedBackcorrunArrayTimer::receiveBuffer == FeedBackcorrunArrayTimer::allReceivedNumber))
			continue;
		...
		SimMessageDDs simdds;
		{
			...
			{
				if ((FeedBackcorrunArrayTimer::index2number[i] & FeedBackcorrunArrayTimer::allReceivedNumber) ==
			...
			simdds.data.push_back((unsigned char)FeedBackcorrunArrayTimer::allReceivedNumber);//低8位
			simdds.data.push_back((unsigned char)(FeedBackcorrunArrayTimer::allReceivedNumber >> 8));//高8位
#if 0
