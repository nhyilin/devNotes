...
#include <QDebug>
#define  TIME_BY_VALUE
using namespace std;
using namespace eprosima;
using namespace fastrtps;
using namespace rtps;
SimCentre::SimCentre(QWidget *parent)
	: QWidget(parent)
...
void SimCentre::Inconnect()
{
	connect(ui.pushButton_publisher, SIGNAL(clicked()), this, SLOT(slot_publisher()));
	connect(ui.pushButton_stop_publisher, SIGNAL(clicked()), this, SLOT(slot_stop_publisher()));
	connect(ui.pushButton_pause_publisher, SIGNAL(clicked()), this, SLOT(slot_pause_publisher()));
	connect(ui.pushButton_cor_publisher, SIGNAL(clicked()), this, SLOT(slot_cor_publisher()));
	connect(ui.pushButton_advance_publisher, SIGNAL(clicked()), this, SLOT(slot_command_pubpublisher()));
	connect(ui.pushButton_node_search,SIGNAL(clicked()),this,SLOT(slot_command_pubpublisher()));
	connect(ui.pushButton_node_statesearch, SIGNAL(clicked()), this, SLOT(slot_command_pubpublisher()));
	connect(ui.pushButton_recover_snapshoot, SIGNAL(clicked()),this,SLOT(slot_skp_publisher()));
...
void SimCentre::slot_pause_publisher()
{
...
void SimCentre::slot_stop_publisher()
{
	int nradio = m_buttongroup->checkedId();
	switch (nradio)
	{
...
void SimCentre::slot_cor_publisher()
{
	int nradio = m_buttongroup->checkedId();
	switch (nradio)
	{
	...
	{//实时推进2
		...
		{
			m_mypublisherrealtimertest = new MyPublisherRealTimerTestfirst();
		}
		#else
...
void SimCentre::slot_only_publisher()
{
...
void SimCentre::slot_command_pubpublisher()
{
...
void SimCentre::slot_skp_publisher()
{
	QPushButton * btn = dynamic_cast<QPushButton*>(sender());
	QString strname = btn->text();
	SimMessageDDs simdds;
...
void SimCentre::slot_recv_pubsubscriber()
{
...
void SimCentre::closeEvent(QCloseEvent *event)
{
