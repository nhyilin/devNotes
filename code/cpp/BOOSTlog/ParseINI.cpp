...
bool ParseINI::AnalyseLine(const string &line, string &key, string &val) {
    if (line.empty()) {
        return false;
    }
...
bool ParseINI::IsCommentChar(char c) {
    switch (c) {
        case COMMENT_CHAR:
            return true;
        default:
...
void ParseINI::PrintConfig(const map<string, string> &mContent) {
