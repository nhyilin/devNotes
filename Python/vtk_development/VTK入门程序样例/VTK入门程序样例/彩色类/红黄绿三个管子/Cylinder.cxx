...
// argv="E:\\VTK_DOC\\VTKData\\Data\\financial.txt";
  //char* fname = vtkTestUtilities::ExpandDataFileName(argc, argv, "E:\\financial.txt");
  // read data
  vtkDataSet *dataSet = ReadFinancialData("financial.txt", "MONTHLY_PAYMENT","INTEREST_RATE",
...
static vtkDataSet *ReadFinancialData(const char* filename, const char *x, const char *y, const char *z, const char *s)
{
  ...
  for (i=0; i<npts; i++)
    {
    xyz[0] = *xV; xyz[1] = *yV; xyz[2] = *zV; 
    newPts->InsertPoint(i, xyz);
    newScalars->InsertValue(i, *sV);
    }
...
static int ParseFile(FILE *file, const char *label, float *data)
{
  ...
  while ( !readData && fscanf(file, "%s", tag) == 1 )
    ...
    if ( ! strcmp(tag,label) )
      ...
      // normalize data
      for (i=0; i<npts; i++) *data = min + *data/(max-min);
      }
    else
      {
