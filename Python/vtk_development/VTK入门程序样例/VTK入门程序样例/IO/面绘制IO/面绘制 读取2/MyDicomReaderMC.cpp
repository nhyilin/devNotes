...
/*
  vtkPolyDataReader *polyReader=vtkPolyDataReader::New();
    polyReader->SetFileName("PolyDataWriter.vtk");
	polyReader->Update();
*/
  vtkSTLReader *polyReader=vtkSTLReader::New();
    polyReader->SetFileName("STLWriter.stl");
	polyReader->Update();
/*
  vtkXMLPolyDataReader *polyReader=vtkXMLPolyDataReader::New();
    polyReader->SetFileName("XMLPolyDataWriter.xpd");
	polyReader->Update();

  vtkMCubesReader *polyReader=vtkMCubesReader::New();
    polyReader->SetFileName("MCubesWriter.tri");
    polyReader->SetLimitsFileName("MCubesWriter.lim");
	polyReader->Update();

  vtkBYUReader *polyReader=vtkBYUReader::New();
    polyReader->SetGeometryFileName("BYUWriter.g");
    polyReader->SetScalarFileName("BYUWriter.s");
//    polyReader->SetDisplacementFileName ("BYUWriter.d");
//	polyReader->SetPartNumber(1);
	polyReader->Update();

  vtkDataSetReader *polyReader=vtkDataSetReader::New();
    polyReader->SetFileName("vtkDataSetWriter.dsw");
	polyReader->Update();
*/
