...
int main (int argc, char **argv)
{
  ...
  // is the root name of the file: quarter.)
	vtkVolume16Reader *v16 = vtkVolume16Reader::New();
                                     //初始化16位图像类型,前缀等
    v16->SetDataDimensions (512,512);//设数据范围大小
    v16->SetImageRange (1,57);//读取图像的文件名 *.1--*.123
    v16->SetDataByteOrderToLittleEndian();//字节格式(字节顺序),如果是littleEndian要倒过来
    v16->SetFilePrefix ("E://CT/ct1/ct");//设置文件目录
    v16->SetDataSpacing (3.2, 3.2, 20);//像素间的间隔
  ...
  vtkActor *skin = vtkActor::New();
    ...
    skin->GetProperty()->SetDiffuseColor(1, .49, .25); //-设颜色 肉色
  ...
  vtkActor *bone = vtkActor::New();
    ...
    bone->GetProperty()->SetDiffuseColor(1, 1, .9412);  //设颜色 银白
